...
model.add(Bidirectional(GRU(128, return_sequences=True, dropout=0.5, recurrent_dropout=0.5)))
...
checkpoint = ModelCheckpoint("testing_weight.hdf5", monitor='val_loss', verbose=1, save_best_only=True, mode='min')
earlystop = EarlyStopping(monitor='val_loss', patience=6, verbose=1, mode="min")
callbacks_list = [checkpoint,earlystop]

model.fit(x_train, y_train, validation_data=(x_test,y_test), epochs=120, batch_size=35, verbose=1)
...
output_df = pd.DataFrame(columns=["Id","Sequence"])
Exist_id = {}
Ids = []
for i in list(X_test_df["Id"]):
    Id = i[:i.rfind("_")] 
...
for i,j in zip(list(X_test_df["Id"]),X_test_phone):
    Id = i[:i.rfind("_")]
    if output_df.loc[Id]["Sequence"] == "":
        output_df.loc[Id]["Sequence"] = str(j)
    elif output_df.loc[Id]["Sequence"][-1] != j :
        output_df.loc[Id]["Sequence"] = str(output_df.loc[Id]["Sequence"]+j)
